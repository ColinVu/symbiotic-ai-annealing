...
def tune(
    ...
    lock_sequence_constraint: bool = False,
    verbose: bool = True,
) -> Dict:
    ...
    if pipeline_mode == "two-stage":
        ...
        if os.path.isfile(cfg_path):
            try:
                ...
                if (
                    not lock_sequence_constraint
                    and cfg.get("use_sequence_constraint") is not None
                ):
                    use_sequence_constraint = bool(cfg.get("use_sequence_constraint"))
                if min_segment_seconds <= 0:
                    min_segment_seconds = float(cfg.get("min_segment_seconds", min_segment_seconds))
                if aruco_persistence_frames <= 0:
                    aruco_persistence_frames = int(cfg.get("aruco_persistence_frames", aruco_persistence_frames))
                if aruco_smoothing_window <= 1:
                    aruco_smoothing_window = int(cfg.get("aruco_smoothing_window", aruco_smoothing_window))
            except Exception:
                pass
    ...
    rows: List[Dict] = []
    _label_names = ["PICK", "CARRY_WITH", "PLACE", "CARRY_EMPTY"]
    best: Dict = {
        "boundary_rmse": float("inf"),
        "macro_f1": -1.0,
        "word_penalty": float(penalties[0]) if penalties else 0.0,
        "grammar_scale": float(scales[0]) if scales else 5.0,
        "samples": 0,
        "coarse_macro_f1": 0.0,
        "interact_macro_f1": 0.0,
        "carry_macro_f1": 0.0,
        "per_class_recall": {k: 0.0 for k in _label_names},
    }
    for p in penalties:
        for s in scales:
            y_true: List[str] = []
            ...
            boundary_rmses: List[float] = []
            for item in cached:
                labels_df = pd.DataFrame(
                    item["labels"], columns=["timestamp_start", "timestamp_end", "state"]
                )
                expected_fine_sequence = (
                    derive_expected_sequence_from_segments(labels_df) if use_sequence_constraint else None
                )
                if pipeline_mode == "legacy":
                    ...
                    if expected_fine_sequence:
                        ...
                        expected_interact_seq = [x for x in expected_fine_sequence if x in INTERACT_STATES]
                        expected_carry_seq = [x for x in expected_fine_sequence if x in CARRY_STATES]
                    ...
                    pred_fine_frame = merge_short_segments(
                        pred_fine_frame,
                        item["frame_numbers"],
                        item["fps"],
            ...
            row = {
                "word_penalty": p,
                "grammar_scale": s,
                "boundary_rmse_seconds": float(np.mean(boundary_rmses)) if boundary_rmses else float("inf"),
                "macro_f1": macro_f1,
                "samples": len(y_true),
            ...
            if verbose:
                print(
                    ...
                    f"boundary_rmse={row['boundary_rmse_seconds']:.4f}s "
            ...
            tiebreak = macro_f1 + coarse_macro_f1 + interact_macro_f1 + carry_macro_f1
            best_tiebreak = (
                best.get("macro_f1", 0.0)
                + best.get("coarse_macro_f1", 0.0)
                + best.get("interact_macro_f1", 0.0)
                + best.get("carry_macro_f1", 0.0)
            )
            if (
                row["boundary_rmse_seconds"] < best["boundary_rmse"]
                or (
                    abs(row["boundary_rmse_seconds"] - best["boundary_rmse"]) < 1e-12
                    and tiebreak > best_tiebreak
                )
            ):
                best = {
                    "boundary_rmse": row["boundary_rmse_seconds"],
                    "macro_f1": macro_f1,
                    "word_penalty": p,
                    "grammar_scale": s,
                    "samples": len(y_true),
    ...
    rows_sorted = sorted(rows, key=lambda r: (r["boundary_rmse_seconds"], -r["macro_f1"]))
    result = {
        "strict_cycle": strict_cycle,
        ...
        json.dump(
            {
                ...
                "strict_cycle": strict_cycle,
                "pipeline_mode": pipeline_mode,
                "use_sequence_constraint": bool(use_sequence_constraint),
                "min_segment_seconds": float(min_segment_seconds),
                "dev_macro_f1": best["macro_f1"],
                "dev_boundary_rmse_seconds": best["boundary_rmse"],
    ...
    if verbose:
        print(
            ...
            f"s={best['grammar_scale']}, boundary_rmse={best['boundary_rmse']:.4f}s, "
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Disable label-derived expected-sequence constraints during tuning (default: enabled).",
    )
    p.add_argument("--min-segment-seconds", type=float, default=0.15, help="Merge predicted runs shorter than this duration.")
...
def main(argv=None) -> None:
    ...
    tune(
        ...
        lock_sequence_constraint=args.no_sequence_constraint,
