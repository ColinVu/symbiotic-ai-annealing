...
class ArucoDetector:
    ...
    def compute_bin_context_channels(
        self,
        frame: np.ndarray,
        hand_position: Tuple[float, float],
    ) -> Tuple[float, float, float]:
        """Pick / place proximity and signed context (compatible with single scalar).

        Returns:
            ``(pick_proximity, place_proximity, signed_context)`` where each
            proximity is in ``[0, 1]`` (sum of distance-weighted marker scores,
            clipped), and ``signed_context`` is ``clip(pick_raw - place_raw, -1, 1)``
            matching :meth:`compute_bin_context_weight` when only pick/place bins exist.
        """
        markers = self.detect_markers(frame)
        if len(markers["ids"]) == 0:
            return (0.0, 0.0, 0.0)
        ...
        for center, bin_type in zip(markers["centers"], markers["types"]):
            ...
            distance = np.linalg.norm(np.array(hand_position) - np.array(center))
            normalized_dist = distance / frame_diag
            proximity = float(np.exp(-normalized_dist * self.distance_decay))
            if bin_type == "pick":
                pick_raw += proximity
            elif bin_type == "place":
                place_raw += proximity

        signed = float(np.clip(pick_raw - place_raw, -1.0, 1.0))
        pick_prox = float(np.clip(pick_raw, 0.0, 1.0))
        place_prox = float(np.clip(place_raw, 0.0, 1.0))
        return (pick_prox, place_prox, signed)
