...
from ..training.geometry import l2_normalize_rows # Import your utility


def _trainer_fit_kwargs_from_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Keyword args for ``WeakSupervisedTrainer.fit`` when iterated-model is enabled."""
    if not bool(config.get("use_iterated_model")):
        return {}
    he = config.get("hand_embeddings_dir")
    if not he:
        raise ValueError("config['hand_embeddings_dir'] is required when use_iterated_model=True")
    return {
        "use_iterated_model": True,
        "hand_embeddings_dir": str(he),
        "refinement_loops": int(config.get("refinement_loops", 3)),
        "sa_iters": int(config.get("sa_iters", 100)),
        ...
        "proxy_energy_margin": float(config.get("proxy_energy_margin", 0.1)),
    }
...
def _apply_pca_to_segments(
    video_segments: Dict[str, Tuple[List[Segment], List[str]]],
    config: Dict[str, Any],
    verbose: bool = True,
) -> Optional[Dict[str, Any]]:
    """
    Apply HandNeutralizer PCA preprocessing to all segment embeddings, then L2-normalize.
    
    Modifies segment embeddings in-place. Returns the neutralizer state dict for model
    persistence, or None if PCA is disabled.
    
    Args:
        video_segments: Dict mapping video_name -> (segments, flat_picklist)
        config: Training config with 'apply_hand_pca', 'hand_embeddings_dir', 'n_components'
        verbose: Print progress messages
        
    Returns:
        HandNeutralizer state dict if PCA was applied, otherwise None
    """
    from ..training.hand_neutralizer import HandNeutralizer
    from ..training.geometry import l2_normalize_rows
    
    if not config.get("apply_hand_pca", False):
        return None
    
    hand_embeddings_dir = config.get("hand_embeddings_dir")
    if not hand_embeddings_dir:
        raise ValueError("apply_hand_pca=True requires hand_embeddings_dir in config")
    
    n_components = int(config.get("n_components", 20))
    
    if verbose:
        print("\n" + "=" * 60)
        print("APPLYING HAND NEUTRALIZER PCA PREPROCESSING")
        print("=" * 60)
        print(f"Hand embeddings directory: {hand_embeddings_dir}")
        print(f"Removing {n_components} PCA components from all embeddings...")
    
    neutralizer = HandNeutralizer(hand_embeddings_dir, n_components=n_components, verbose=verbose)
    
    # Apply PCA to all segment embeddings, then L2-normalize
    for video_name, (segments, _) in video_segments.items():
        for seg in segments:
            if seg.is_placeholder or seg.embeddings.size == 0:
                continue
            # Step 1: Apply PCA neutralization
            seg.embeddings = neutralizer.neutralize(seg.embeddings)
            # Step 2: L2-normalize each row (frame embedding)
            seg.embeddings = l2_normalize_rows(seg.embeddings)
    
    if verbose:
        print("✓ PCA preprocessing complete (embeddings neutralized & L2-normalized)\n")
    
    return neutralizer.state_dict()
...
def run_multi_video_training_from_cache(
    ...
    trainer = WeakSupervisedTrainer(
        ...
        min_temp=float(config.get("min_temp", 0.05)),
    )
    
    # Apply HandNeutralizer PCA preprocessing if requested
    neutralizer_state = _apply_pca_to_segments(video_segments, config, verbose)
    if neutralizer_state:
        trainer.hand_neutralizer_state = neutralizer_state
...
def run_video_training(
    ...
    # Apply HandNeutralizer PCA preprocessing if requested
    video_segments_dict = {video_name: (segments, flat_picklist)}
    neutralizer_state = _apply_pca_to_segments(video_segments_dict, config, verbose)
    if neutralizer_state:
        trainer.hand_neutralizer_state = neutralizer_state
...
def run_incremental_training(
    ...
    # Apply HandNeutralizer PCA preprocessing if requested
    video_segments_dict = {video_name: (segments, picklist)}
    neutralizer_state = _apply_pca_to_segments(video_segments_dict, save_config, verbose)
    if neutralizer_state:
        trainer.hand_neutralizer_state = neutralizer_state
