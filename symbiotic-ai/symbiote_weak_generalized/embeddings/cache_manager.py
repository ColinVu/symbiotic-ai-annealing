...
def _frame_seg_cache_path(label: str, frame_number: int, cache_dir: str) -> str:
    ...
    return os.path.join(cache_dir, seg_cache_filename)


def load_frame_seg_from_cache(label: str, frame_number: int, cache_dir: str) -> Optional[np.ndarray]:
    """Load cached hand crop image for a video frame, if present."""
    try:
        seg_cache_path = _frame_seg_cache_path(label, frame_number, cache_dir)
        if os.path.exists(seg_cache_path):
            return np.load(seg_cache_path)
    except Exception:
        pass
    return None


def load_frame_from_cache(label: str, frame_number: int, cache_dir: str) -> Optional[np.ndarray]:
    """
    Load embedding from cache for a video frame.
