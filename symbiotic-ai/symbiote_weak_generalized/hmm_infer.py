...
from symbiote_weak.state_detection.aruco_detection import ArucoDetector  # noqa: E402
from symbiote_weak.state_detection.config import DEFAULT_HTK_CONFIG       # noqa: E402
from symbiote_weak.state_detection.feature_extraction import FeatureExtractor  # noqa: E402
from symbiote_weak.state_detection.htk_interface import HTKStateDetector  # noqa: E402
from symbiote_weak.state_detection.two_stage import (  # noqa: E402
    CARRY_STATES,
    COARSE_STATES,
    INTERACT_STATES,
    derive_expected_sequence_from_segments,
    apply_feature_mask,
    map_fine_to_coarse,
    merge_short_segments,
    project_labels_to_expected_sequence,
...
def run_inference(
    ...
    use_sequence_constraint: Optional[bool] = None,
    sequence_label_csv: Optional[str] = None,
    ...
    if os.path.isfile(pipeline_cfg_path):
        try:
            ...
            if use_sequence_constraint is None:
                use_sequence_constraint = bool(pipeline_cfg.get("use_sequence_constraint", True))
            if min_segment_seconds <= 0:
                min_segment_seconds = float(pipeline_cfg.get("min_segment_seconds", min_segment_seconds))
            if aruco_persistence_frames <= 0:
                aruco_persistence_frames = int(pipeline_cfg.get("aruco_persistence_frames", aruco_persistence_frames))
            if aruco_smoothing_window <= 1:
                aruco_smoothing_window = int(pipeline_cfg.get("aruco_smoothing_window", aruco_smoothing_window))
        except Exception:
            if verbose:
    ...
    expected_fine_sequence: Optional[list[str]] = None
    sequence_path: Optional[str] = sequence_label_csv
    if use_sequence_constraint and not sequence_path:
        auto_csv = Path(video_path).with_name(f"{Path(video_path).stem}.csv")
        if auto_csv.is_file():
            sequence_path = str(auto_csv)
            if verbose:
                print(f"[hmm_infer] Same-stem label CSV for sequence constraint: {sequence_path}")
    if use_sequence_constraint and sequence_path and os.path.isfile(sequence_path):
        try:
            expected_df = pd.read_csv(sequence_path)
            expected_fine_sequence = derive_expected_sequence_from_segments(expected_df)
            if verbose and expected_fine_sequence:
                print(f"[hmm_infer] Label-derived sequence constraint ({len(expected_fine_sequence)} states).")
        except Exception:
            if verbose:
                print("[hmm_infer] WARNING: failed to parse sequence label CSV; constraint disabled.")
            expected_fine_sequence = None
    elif use_sequence_constraint and verbose:
        print(
            "[hmm_infer] Sequence constraint enabled but no label CSV found "
            "(use --sequence-label-csv or place <video_stem>.csv beside the video)."
        )
    ...
    if pipeline_mode == "legacy":
        ...
        segments: pd.DataFrame = htk_detector.decode(
            ...
            expected_sequence=expected_fine_sequence,
        )
    else:
        ...
        expected_coarse_seq = None
        expected_interact_seq = None
        expected_carry_seq = None
        if expected_fine_sequence:
            expected_coarse_seq = []
            for st in expected_fine_sequence:
                c = map_fine_to_coarse(st)
                if not expected_coarse_seq or expected_coarse_seq[-1] != c:
                    expected_coarse_seq.append(c)
            expected_interact_seq = [s for s in expected_fine_sequence if s in INTERACT_STATES]
            expected_carry_seq = [s for s in expected_fine_sequence if s in CARRY_STATES]
        coarse_segments = coarse_detector.decode(
            ...
            grammar_scale=grammar_scale,
            strict_cycle=strict_cycle,
            expected_sequence=expected_coarse_seq if use_sequence_constraint else None,
        )
        ...
        fine_frame_labels = decode_subtype_with_runs(
            ...
            expected_interact_sequence=expected_interact_seq if use_sequence_constraint else None,
            expected_carry_sequence=expected_carry_seq if use_sequence_constraint else None,
        )
        fine_frame_labels = merge_short_segments(
            fine_frame_labels,
            frame_numbers,
            fps,
            min_segment_seconds=min_segment_seconds,
        )
        if use_sequence_constraint and expected_fine_sequence:
            fine_frame_labels = project_labels_to_expected_sequence(
                fine_frame_labels,
                expected_fine_sequence,
            )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Disable label-derived sequence constraints (default: on if pipeline_config / same-stem CSV allows).",
    )
    p.add_argument(
        "--sequence-label-csv",
        default=None,
        help="Per-video label CSV for expected action sequence (default: <video_stem>.csv next to the video if present).",
    )
    p.add_argument("--min-segment-seconds", type=float, default=0.15, help="Merge decoded runs shorter than this duration.")
...
def main(argv=None):
    ...
    try:
        ...
        run_inference(
            ...
            use_sequence_constraint=False if args.no_sequence_constraint else None,
            sequence_label_csv=args.sequence_label_csv,
