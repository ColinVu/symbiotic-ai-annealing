...
def _load_cached_features(
    ...
    expected = _cache_key(video_path, frame_skip, blur_threshold, feature_mask)
...
def _save_cached_features(
    ...
    manifest[stem] = _cache_key(video_path, frame_skip, blur_threshold, feature_mask)
...
def train_state_detector(
    ...
    for video_path, annotation_path in zip(video_paths, annotation_paths):
        ...
        features = _load_cached_features(
            cache_dir, manifest, video_path, frame_skip, blur_threshold, feature_mask, verbose
        )
        ...
        if features is not None:
            ...
            _save_cached_features(
                cache_dir, manifest, video_path, features, frame_skip, blur_threshold, feature_mask
            )
            cache_misses += 1
