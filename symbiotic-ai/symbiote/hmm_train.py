...
from symbiote.state_detection.training import train_state_detector  # noqa: E402
from symbiote.eaf_to_csv import convert_directory as _convert_eaf_dir  # noqa: E402
from symbiote.hmm_tune import tune as tune_decode_params  # noqa: E402
...
def main(argv=None):
    ...
    try:
        ...
        if resolved_mask:
            if min(resolved_mask) < 0 or max(resolved_mask) > 14:
                raise ValueError("Feature indices must be in [0, 14].")
        for stage_name, stage_mask in [
            ...
            if stage_mask:
                if min(stage_mask) < 0 or max(stage_mask) > 14:
                    raise ValueError(f"{stage_name} feature indices must be in [0, 14].")
