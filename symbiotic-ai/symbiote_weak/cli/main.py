...
Examples:
  ...
  # Hyperparameter sweep (scores ILR segment labels vs ground_truth.csv for all training stems)
...
"""
    ...
    sweep_parser = subparsers.add_parser(
        "sweep",
        help="Hyperparameter sweep: train-from-cache per config; rank by ILR label accuracy vs ground_truth.csv (all training stems)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ...
    sweep_parser.add_argument(
        ...
        default="random",
        choices=["grid", "random"],
        help="Exhaustive grid or random sampling over PARAM_GRID",
    )
    sweep_parser.add_argument(
        "--num-samples",
        type=int,
    ...
    if args.command == "train":
        ...
        from ..experiments.sweep_config import generate_configs, grid_size
        ...
        print(
            f"Training runs: {len(combos)}; scoring = ILR segment labels vs ground truth "
            f"({len(overlap)} video(s) with GT columns)"
        )
        ...
        for rec in results:
            ...
            try:
                if trainer is None:
                    raise RuntimeError("Missing trainer on successful run")
                ev = score_training_assignments_vs_ground_truth(
                    trainer,
                    ground_truth_csv=ground_truth_csv,
                    video_stems=training_stems,
                )
        ...
        if best:
            print("\nBest run (micro-averaged assignment top-1 vs ground truth):")
            print(f"  run_dir: {best.get('run_dir')}")
            print(f"  segment_top1_accuracy: {best.get('segment_top1_accuracy')}")
        else:
